...
function fmtDate(iso: string) {
  // iso puede venir con timezone; esto lo hace legible
...
export default function SolicitudesClient({ solicitudes }: { solicitudes: Solicitud[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return (solicitudes || []).filter((x) => {
      const hayStatus = status ? String(x.status || "").toLowerCase() === status.toLowerCase() : true;

      if (!s) return hayStatus;
  ...
  const wrap: CSSProperties = { display: "grid", gap: 12 };
